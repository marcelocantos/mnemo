...
**Status:** archived exploration. Captures an idea that surfaced while fixing a
silent stall in the `ytt` playlist-ingest automation (June 2026). Owner: Marcelo.

Superseded as written. The generic extension machinery this sketch assumed was
built instead as the 🎯T102 plugin system, which is where an automation-liveness
capability now belongs — `bullseye.yaml` records it as the motivating consumer
of that system: "it exercises nearly every extension point (signal-source +
reconciler + check + notifier + UI + config) and needs almost no new code."

The original target id (T98) is **unusable** — it collides with git history —
so the work is to be re-raised under a fresh id depending on 🎯T102. This
document is kept for its failure analysis and its poll-vs-push design fork,
both of which still apply; treat the implementation sketch below as history
rather than a plan.
