...
- `mnemo_query` — SQL SELECT/WITH or sqldeep nested syntax (FROM ... SELECT { }) against the transcript database. Call `mnemo_query(describe=true)`, or read the `mnemo://schema` resource, for the table/column catalogue — it is generated from the live database, so unlike the old inline listing it cannot drift (🎯T156). Read entries through `entries_v`, message text through `messages_v`, doc content through `docs_v`.
