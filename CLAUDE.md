...
**Vault theme clustering (🎯T64.8).** Two independent egress surfaces,
each its own opt-in, both off by default:

- **Voyage embeddings.** The embeddings clustering engine fetches dense
  vectors from Voyage AI. Gated on `vault_clustering.engine:
  "embeddings"` **and** `VOYAGE_API_KEY` present; the key alone does
  nothing. Vectors are cached in `cluster_embeddings` keyed by
  `(doc_kind, entity_id, content_hash, provider, model, model_version)`,
  so a warm pass makes no call.
- **Haiku labelling.** Cluster labels can be generated by Claude Haiku.
  Gated on `vault_clustering.label.engine: "llm"` **and**
  `ANTHROPIC_API_KEY` present.

The default engine (heuristic TF-IDF + single-link) and the default
label chain (user-anchored → bigram) are **fully local**: with both
keys in the environment but neither opt-in set, a clustering pass
constructs no provider and makes zero outbound requests (the two-key
matrix is a regression test). Opted-in-but-no-key degrades to the local
path for that pass and reports the reason in
`mnemo_vault_status`'s Theme-clustering warnings.
